...
- **Static 2**: starts with the mesh from Static 1 and applies a smooth
  ...
  in the configuration in `Allrun`.
    ...
    This distortion is applied to all mesh points except those specified as fixed
    in the `$FOAM_CASE/system/smoothlyDistortedMeshDict`.

    ```foam
    fixedPatches (walls anotherPatch ...);
    ```

    Also, the user can control the magnitude of the distortion in each of the x,
    y, and z direction, through the $A$ vector entry in the mentioned dictionary.

    ```foam
    A    (0.02 0.03 0);
    ```

    A positive value in the x-direction moves a mesh point in the positive
    x-direction, and similarly for the y- and z-direction. Since this is a 2-D
    problem, the z-component is set to `0`, indicating no distortion along the
    z-axis. For example, the vector `(0.2 0.3 0)` moves mesh points diagonally
    toward the northeast.

    As can be seen from the plots, the value of the function $f$ increases toward
    the center of the domain in the $x-y$ plane. This trend is also evident when
    comparing the distorted mesh with the original orthogonal mesh.

    ![Comparison between the base orthogonal mesh and the distorted mesh using bump function](./images/bumped_vs_orthogonal.png)

    Additionally, since the derivative of the bump function is zero at the domain
    boundary, refining the mesh should result in little to no distortion at the
    boundary.

    In the limit of mesh refinement, the local face non-orthogonality goes to
    zero; that is, the entire mesh becomes effectively orthogonal for fine meshes.
